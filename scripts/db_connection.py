...
def mysql_connection(env,customer):
    ...
    mysql_server = db[customer]['my' + env][0]['mysql_server']
    mysql_username = db[customer]['my' + env][0]['mysql_username']
    mysql_password = db[customer]['my' + env][0]['mysql_password']
    mysql_db = db[customer]['my' + env][0]['mysql_database']
    try:
...
def sql_connection_message_storage(env, customer):
    db = abs_data_base_credentials()
    sql_server = db[customer][env][0]['sql_server']
    sql_username = db[customer][env][0]['sql_username']
    sql_password = db[customer][env][0]['sql_password']
    sql_db = db[customer][env][0]['sql_database_message_store']
    try:
...
def sql_connection_adapter(env, customer):
    ...
    sql_username = db[customer][env][0]['sql_username']
    sql_password = db[customer][env][0]['sql_password']
    sql_db = db[customer][env][0]['sql_database_adapter']
    try:
